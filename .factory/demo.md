...
- Browser isolation: the demo writes no personal release data to localStorage, sessionStorage, IndexedDB, or cookies. Its build-versioned `ios-review-gate-<build-id>` cache contains same-origin static shell files only. License storage is separate and is not read in demo mode.
