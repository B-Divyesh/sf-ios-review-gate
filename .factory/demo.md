...
- Website reset: choose **Reset demo**. The web demo writes no personal release data to localStorage, sessionStorage, IndexedDB, or cookies. Its `ios-review-gate-v3` service-worker cache stores only the static offline shell in Cache Storage. License storage is separate and is not read by the demo.
